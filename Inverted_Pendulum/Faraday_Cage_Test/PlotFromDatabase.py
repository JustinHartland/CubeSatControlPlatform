...
def fetch_imu_data_for_trial(database_name, trial_id):
    ...
    # Fetching angle_x and encoder_velocity
    cur.execute("SELECT time, angle_x, encoder_velocity FROM imu_data WHERE trial_id=?", (trial_id,))
    data = cur.fetchall()
...
def plot_imu_data(data):
    # Splitting data into columns for plotting
    times, angles_x, encoder_velocities = zip(*data)
    ...
    plt.title("IMU Angles and Encoder Velocity over Time")
    plt.xlabel("Time")
    plt.ylabel("Angle (degrees) / Encoder Velocity")
    plt.legend()
